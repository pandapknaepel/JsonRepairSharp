...
namespace JsonRepairSharp_GUI
{
    public partial class CompareJsonGui : Form
    {
        ...
        private void btFirst_Click(object sender, EventArgs e)
        {
            if (ofdFile.ShowDialog() == DialogResult.OK)
            {
                ...
                if (File.Exists(tbFirstFile.Text)) LoadLeft();
            }
        }
        ...
        private void LoadLeft()
        {
            ...
            fctb1.Language = fctb2.Language = Language.JSON;

            var source1 = Lines.Load(_linesLeft);
            var source2 = Lines.Load(_linesRight);
            ...
            tbSecondFile.Enabled = btSecond.Enabled = buttonSave.Enabled = true;

            var inputFile = tbFirstFile.Text;
            tbSecondFile.Text = $"{Path.GetDirectoryName(inputFile)}\\{Path.GetFileNameWithoutExtension(inputFile)}-fixed{Path.GetExtension(inputFile)}";
        ...
        {
            var fileName = tbSecondFile.Text;
            if (!Directory.Exists(Path.GetDirectoryName(fileName)))
            {
                MessageBox.Show(this, "Please select an existing directory", "Directory not existing");
                return;
            }
    ...
    namespace DiffMergeStuffs
    {
        ...
        public class Lines : List<Line>, IEquatable<Lines>
        {
            ...
            public static Lines Load(string[] stringArray)
            {
                Lines lines = new Lines();
            ...
            public void Merge(Lines lines)
            {
                ...
                //We run the algorithm to find the longest common subsequence (LCS)."
                diff.RunDiff();
            }
            ...
            {
                var text = "";
                foreach (var list in this) { text += list.ToString() + "\n"; }
                return text;
            }
            ...
            public void FromText(string text)
            {
                ...
                foreach (var stringLine in strings) { Add(new Line(stringLine)); }
            }

        }

        /// <summary>
        /// "A line containing multiple conflicting versions
        /// </summary>
